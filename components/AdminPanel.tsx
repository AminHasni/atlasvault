...
const getStatusColor = (status: OrderStatus) => {
  switch (status) {
    case 'pending_whatsapp': return 'bg-amber-500/10 text-amber-400 border-amber-500/20';
...
export const AdminPanel: React.FC<AdminPanelProps> = ({ services, categories, onUpdate, notify, activeTab }) => {
  ...
  return (
    <div className="w-full space-y-6">
      {/* ... (Previous JSX for dashboard, tabs) ... */}
        ...
        <div className="space-y-6 animate-in fade-in duration-300">
           ...
           <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
              <div className="rounded-xl border border-slate-700 bg-slate-800/50 p-6">
                 ...
                 <p className="mt-2 text-2xl font-bold text-white">${stats.totalRevenue.toFixed(2)}</p>
              ...
              <div className="rounded-xl border border-slate-700 bg-slate-800/50 p-6">
                 ...
                 <p className="mt-2 text-2xl font-bold text-white">{stats.pendingOrders}</p>
              ...
              <div className="rounded-xl border border-slate-700 bg-slate-800/50 p-6">
                 ...
                 <p className="mt-2 text-2xl font-bold text-white">{stats.activeServices}</p>
                 <p className="text-xs text-slate-500">Currently listed in catalog</p>
              </div>
           </div>
           ...
           <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
              ...
              <div className="rounded-xl border border-slate-700 bg-slate-800/50 p-6">
                 <h3 className="mb-4 flex items-center gap-2 text-lg font-semibold text-white">
                    <TrendingUp className="h-5 w-5 text-indigo-400" /> Most Requested Services
                 </h3>
                 <div className="space-y-4">
                    {stats.topServices.length > 0 ? (
                       stats.topServices.map((item, index) => (
                          <div key={index} className="flex items-center justify-between border-b border-slate-700/50 pb-2 last:border-0 last:pb-0">
                             <span className="font-medium text-slate-300 truncate max-w-[70%]">{item.name}</span>
                             <span className="rounded-full bg-slate-700 px-2.5 py-0.5 text-xs font-bold text-white">{item.count} orders</span>
                          </div>
                       ))
                    ) : (
              ...
              <div className="rounded-xl border border-slate-700 bg-slate-800/50 p-6">
                 <h3 className="mb-4 flex items-center gap-2 text-lg font-semibold text-white">
                    <PieChart className="h-5 w-5 text-emerald-400" /> Orders by Category
                 </h3>
                 <div className="space-y-4">
      ...
      {activeTab === 'services' && (
        <div className="animate-in fade-in duration-300 space-y-4">
          ...
          <div className="overflow-hidden rounded-xl border border-slate-700 bg-slate-800/50 shadow-sm">
                    ...
                    <tr key={service.id} className="hover:bg-slate-800/50 transition-colors">
                      <td className="px-6 py-4">
                         <button 
                           ...
                           title="Toggle Status"
                           className={`rounded-full p-1.5 transition-colors ${service.active ? 'bg-emerald-500/10 text-emerald-400 hover:bg-emerald-500/20' : 'bg-red-500/10 text-red-400 hover:bg-red-500/20'}`}
                         >
                      ...
                      <td className="px-6 py-4 font-medium text-white">{service.currency}{service.price.toFixed(2)}</td>
                      <td className="px-6 py-4 text-right">
                        <div className="flex justify-end gap-2">
                          <button 
                            onClick={() => handleEditClick(service)}
                            className="rounded-lg p-2 text-indigo-400 hover:bg-indigo-500/10 transition-colors"
                            title="Edit"
                          >
                        ...
                        return (
                            <tr key={cat.id} className="hover:bg-slate-800/50 transition-colors">
                                <td className="px-6 py-4 text-center">
                                    <div className={`mx-auto flex h-8 w-8 items-center justify-center rounded-lg bg-slate-700 ${cat.color.replace('text-', 'text-')}`}>
                   ...
                   <tbody className="divide-y divide-slate-700">
                     {filteredOrders.map((order) => (
                       <tr key={order.id} className="hover:bg-slate-800/50 transition-colors">
                         ...
                         <td className="px-6 py-4">
                           <div className="font-medium text-white">{order.serviceName}</div>
                           <div className="text-xs text-indigo-400">{order.category}</div>
                         </td>
                         <td className="px-6 py-4 max-w-xs">
                           <div className="font-medium text-slate-200">{order.customerEmail || 'N/A'}</div>
        ...
        {viewingOrder && (
          <div className="space-y-6">
             <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-4">
                   <div className="flex items-start gap-3 rounded-lg bg-slate-800/50 p-3 border border-slate-800">
                      <Tag className="h-5 w-5 text-indigo-400 mt-0.5" />
                      <div>
                        <p className="text-xs text-slate-500 uppercase">Service</p>
                        <p className="font-semibold text-white">{viewingOrder.serviceName}</p>
                        <p className="text-xs text-indigo-400">{viewingOrder.category}</p>
                      </div>
                   </div>
                   ...
                   <div className="flex items-start gap-3 rounded-lg bg-slate-800/50 p-3 border border-slate-800">
                      ...
                      <div>
                        ...
                        <p className="font-semibold text-white">{viewingOrder.currency}{viewingOrder.price.toFixed(2)}</p>
                      </div>
                   </div>
                   ...
                   <div className="flex items-start gap-3 rounded-lg bg-slate-800/50 p-3 border border-slate-800">
                      ...
                      <div>
                        ...
                        <p className="font-medium text-slate-200">{new Date(viewingOrder.createdAt).toLocaleString()}</p>
                      </div>
                   </div>
                </div>
                ...
                <div className="space-y-4">
                   <div className="rounded-lg bg-slate-800/50 p-3 border border-slate-800 h-full">
                      ...
                      <div className="text-sm space-y-2">
                        <p><span className="text-slate-500">Email:</span> <span className="text-white">{viewingOrder.customerEmail || 'N/A'}</span></p>
                        <p><span className="text-slate-500">Phone:</span> <span className="text-white">{viewingOrder.customerPhone || 'N/A'}</span></p>
                        <div className="border-t border-slate-700 pt-2 mt-2">
                           <p className="text-xs text-slate-500 mb-1">Provided Details:</p>
                           <p className="text-slate-300 whitespace-pre-wrap">{viewingOrder.customerInfo.split('Details:')[1]?.trim() || viewingOrder.customerInfo}</p>
                        </div>
                      </div>
                   </div>
                </div>
             ...
             <div className="pt-4 border-t border-slate-800">
                <label className="mb-2 block text-sm font-medium text-slate-400 flex items-center gap-2">
                      ...
                      { id: 'cancelled', label: 'Cancelled', icon: XCircle }
                       ...
                       <button
                         ...
                         className={`flex flex-col items-center justify-center p-3 rounded-lg border transition-all ${
                            ...
                            ? getStatusColor(status.id as OrderStatus) + ' ring-1 ring-offset-1 ring-offset-slate-900 ring-slate-500'
                            : 'bg-slate-800 border-slate-700 text-slate-500 hover:bg-slate-800/80 hover:text-slate-300'
                         }`}
                       >
